...
def test_confirmation_sms(phone_number="+48123456789"):
    """Test wysyłki SMS potwierdzającego."""
...
def test_reminder_sms(phone_number="+48123456789"):
    """Test wysyłki SMS przypominającego."""
...
def create_time_slots():
    """
    Tworzy sloty na kolejne 90 dni (9:00-16:30 co 30 minut).
    Nie tworzy duplikatów. Jeśli w bazie są przypadkowe duplikaty, usuwa je.
    """
    start_date = timezone.localdate()
    end_date = start_date + timedelta(days=90)

    for single_date in (start_date + timedelta(n) for n in range((end_date - start_date).days + 1)):
        if single_date.weekday() in [5, 6]:  # pomijamy weekendy
            continue
        ...
        for hour in range(9, 17):
            for minute in (0, 30):
                ...
                duplicates = TimeSlot.objects.filter(date=single_date, time=slot_time)
                if duplicates.count() > 1:
                    # zachowaj tylko pierwszy
                    for dup in duplicates[1:]:
                        dup.delete()

                # --- TWORZENIE NOWEGO SLOTU, JEŻELI NIE ISTNIEJE ---
